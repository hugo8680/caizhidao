...
export function FinanceTools({ toolId }: { toolId: string }) {
  ...
  return (
    <ToolContext.Provider value={{ values, set }}><section className="tool-calculator-grid single-tool">
      ...
      {toolId === 'real-return' && <article className="tool-calc"><CardTitle id="real-return" /><div className="tool-fields"><Field id="rr" label="名义收益率" suffix="%" /><Field id="ri" label="通胀率" suffix="%" /></div><div className="tool-result green"><span>实际购买力收益率</span><strong>{percent(realReturn)}</strong><p>使用精确 Fisher 关系，而非简单相减。</p></div></article>}

      {toolId === 'cagr' && <article className="tool-calc"><CardTitle id="cagr" /><div className="tool-fields"><Field id="cs" label="期初金额" suffix="元" /><Field id="ce" label="期末金额" suffix="元" /><Field id="cyears" label="经过年数" suffix="年" /></div><div className="tool-result"><span>复合年化增长率</span><strong>{percent(cagr)}</strong><p>它平滑了中间路径，不代表每一年都获得相同收益。</p></div></article>}
      ...
      {toolId === 'position' && <article className="tool-calc"><CardTitle id="position" /><div className="tool-fields"><Field id="pp" label="组合总资产" suffix="元" /><Field id="pr" label="单笔可承受损失" suffix="%" /><Field id="ps" label="止损距离" suffix="%" /><Field id="pprice" label="每单位价格" suffix="元" /></div><div className="tool-result coral"><span>最大仓位金额</span><strong>{money(positionValue)}</strong><p>约 {Number.isFinite(positionShares) ? positionShares.toLocaleString('zh-CN') : '—'} 单位 · 若跳空或流动性不足，实际损失可能超过设定值。</p></div></article>}

      {toolId === 'fee-impact' && <article className="tool-calc"><CardTitle id="fee-impact" /><div className="tool-fields"><Field id="fi" label="初始本金" suffix="元" /><Field id="fa" label="每年新增" suffix="元" /><Field id="fr" label="费用前收益率" suffix="%" /><Field id="fy" label="投资年数" suffix="年" /><Field id="f1" label="低费用率" suffix="%" /><Field id="f2" label="高费用率" suffix="%" /></div><div className="tool-result"><span>长期费用差异</span><strong>{money(lowFee - highFee)}</strong><p>低费用终值 {money(lowFee)}；高费用终值 {money(highFee)}。未计税费与跟踪误差。</p></div></article>}
