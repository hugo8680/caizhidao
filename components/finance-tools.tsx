...
function ToolActions({ onCalculate, onReset }: { onCalculate: () => void; onReset: () => void }) {
  return <div className="tool-actions"><button type="button" onClick={onCalculate}>计算</button><button type="button" onClick={onReset}>重置</button></div>;
}
...
export function FinanceTools({ toolId }: { toolId: string }) {
  ...
  const set = (key: string, value: string) => setDraftValues((current) => ({ ...current, [key]: Number(value) }));
  const calculate = () => setValues({ ...draftValues });
  const reset = () => {
  ...
  return (
    <ToolContext.Provider value={{ values: draftValues, set }}><section className="tool-calculator-grid single-tool">
      {toolId === 'compound' && <article className="tool-calc"><CardTitle id="compound" /><div className="tool-fields"><Field id="cp" label="初始本金" suffix="元" /><Field id="cm" label="每月投入" suffix="元" /><Field id="cr" label="年收益率" suffix="%" /><Field id="cy" label="投资年数" suffix="年" /></div><ToolActions onCalculate={calculate} onReset={reset} /><div className="tool-result"><span>预计终值</span><strong>{money(compound)}</strong><p>累计投入 {money(n('cp') + n('cm') * n('cy') * 12)} · 估算增长 {money(compound - n('cp') - n('cm') * n('cy') * 12)}</p></div></article>}

      {toolId === 'real-return' && <article className="tool-calc"><CardTitle id="real-return" /><div className="tool-fields"><Field id="rr" label="名义收益率" suffix="%" /><Field id="ri" label="通胀率" suffix="%" /></div><ToolActions onCalculate={calculate} onReset={reset} /><div className="tool-result green"><span>实际购买力收益率</span><strong>{percent(realReturn)}</strong><p>使用精确 Fisher 关系，而非简单相减。</p></div></article>}

      {toolId === 'cagr' && <article className="tool-calc"><CardTitle id="cagr" /><div className="tool-fields"><Field id="cs" label="期初金额" suffix="元" /><Field id="ce" label="期末金额" suffix="元" /><Field id="cyears" label="经过年数" suffix="年" /></div><ToolActions onCalculate={calculate} onReset={reset} /><div className="tool-result"><span>复合年化增长率</span><strong>{percent(cagr)}</strong><p>它平滑了中间路径，不代表每一年都获得相同收益。</p></div></article>}

      {toolId === 'loan' && <article className="tool-calc"><CardTitle id="loan" /><div className="tool-fields"><Field id="lp" label="贷款本金" suffix="元" /><Field id="lr" label="年利率" suffix="%" /><Field id="ly" label="贷款期限" suffix="年" /></div><ToolActions onCalculate={calculate} onReset={reset} /><div className="tool-result coral"><span>等额本息月供</span><strong>{money(monthlyLoan)}</strong><p>总利息约 {money(totalLoanInterest)} · 总还款约 {money(monthlyLoan * loanMonths)}</p></div></article>}

      {toolId === 'early-repay' && <article className="tool-calc wide"><CardTitle id="early-repay" /><div className="tool-fields six"><Field id="epb" label="当前剩余本金" suffix="元" /><Field id="ep" label="本次提前还款" suffix="元" /><Field id="er" label="贷款年利率" suffix="%" /><Field id="emths" label="剩余期数" suffix="月" /><Field id="eir" label="替代投资年收益" suffix="%" /><Field id="efee" label="提前还款费用" suffix="%" /></div><ToolActions onCalculate={calculate} onReset={reset} /><div className="tool-result"><span>{prepayment === 0 ? '尚未计入提前还款' : earlyRepayDifference < 1 ? '两种情景的期末资产接近' : retainedFundsAtEnd > prepaidFundsAtEnd ? '保留资金情景的期末资产较高' : '提前还款情景的期末资产较高'}</span><strong>{money(earlyRepayDifference)}</strong><p>按剩余期限不变重算：月供约从 {money(earlyPayment)} 降至 {money(reducedPayment)}；{feeAdjustedSaving >= 0 ? <>扣除提前还款费用后，名义融资成本约节省 {money(feeAdjustedSaving)}</> : <>提前还款费用比名义利息节省高约 {money(Math.abs(feeAdjustedSaving))}</>}。比较假设把降低的月供逐月投资，并将投资收益视为扣税费后的固定情景值；实际收益存在风险。</p></div></article>}

      {toolId === 'saving-goal' && <article className="tool-calc"><CardTitle id="saving-goal" /><div className="tool-fields"><Field id="sg" label="目标金额" suffix="元" /><Field id="sc" label="已有资金" suffix="元" /><Field id="sr" label="年收益率" suffix="%" /><Field id="sy" label="目标年数" suffix="年" /></div><ToolActions onCalculate={calculate} onReset={reset} /><div className="tool-result green"><span>每月需要投入</span><strong>{money(monthlyGoal)}</strong><p>按月末投入估算。收益率越不确定，越应预留安全余量。</p></div></article>}

      {toolId === 'emergency' && <article className="tool-calc"><CardTitle id="emergency" /><div className="tool-fields"><Field id="em" label="每月必要支出" suffix="元" /><Field id="ecov" label="自定覆盖月数" suffix="月" /><Field id="eextra" label="额外应急支出" suffix="元" /><Field id="eliquid" label="现有安全储备" suffix="元" /></div><ToolActions onCalculate={calculate} onReset={reset} /><div className="tool-result"><span>目标储备 · {emergencyMonths} 个月必要支出加额外风险</span><strong>{money(emergencyTarget)}</strong><p>现有可立即动用的安全储备 {money(n('eliquid'))}，距离目标还差 {money(emergencyShortfall)}。覆盖月数由你根据收入恢复时间、家庭责任和保险选择，工具不使用任意评分替你下结论。</p></div></article>}

      {toolId === 'retirement' && <article className="tool-calc wide"><CardTitle id="retirement" /><div className="tool-fields six"><Field id="ra" label="当前年龄" suffix="岁" /><Field id="rt" label="退休年龄" suffix="岁" /><Field id="re" label="当前月支出" suffix="元" /><Field id="rinfl" label="长期通胀" suffix="%" /><Field id="rexist" label="已有养老资金" suffix="元" /><Field id="rret" label="积累期收益率" suffix="%" /><Field id="rw" label="计划提取率" suffix="%" /></div><ToolActions onCalculate={calculate} onReset={reset} /><div className="tool-result coral"><span>退休目标约 {money(retirementTarget)} · 缺口 {money(retirementGap)}</span><strong>每月需积累 {money(retirementMonthly)}</strong><p>{retirementYears} 年后月支出约 {money(retirementExpense)}。提取率只是规划假设，不是安全保证。</p></div></article>}

      {toolId === 'dcf' && <article className="tool-calc wide"><CardTitle id="dcf" /><div className="tool-fields six"><Field id="dfcf" label="当前自由现金流" suffix="元" /><Field id="dg" label="预测期增长" suffix="%" /><Field id="dr" label="折现率" suffix="%" /><Field id="dy" label="预测年数" suffix="年" /><Field id="dtg" label="永续增长率" suffix="%" /><Field id="dcash" label="现金" suffix="元" /><Field id="ddebt" label="有息负债" suffix="元" /></div><ToolActions onCalculate={calculate} onReset={reset} /><div className="tool-result green"><span>简化股权价值</span><strong>{money(equityValue)}</strong><p>折现率必须高于永续增长率。建议改变增长与折现率做情景分析。</p></div></article>}

      {toolId === 'bond' && <article className="tool-calc"><CardTitle id="bond" /><div className="tool-fields"><Field id="bf" label="债券面值" suffix="元" /><Field id="bc" label="年票息率" suffix="%" /><Field id="bret" label="到期收益率" suffix="%" /><Field id="by" label="剩余期限" suffix="年" /></div><ToolActions onCalculate={calculate} onReset={reset} /><div className="tool-result"><span>理论价格</span><strong>{money(bondPrice)}</strong><p>{bondPrice > n('bf') ? '收益率低于票息率，债券呈溢价。' : bondPrice < n('bf') ? '收益率高于票息率，债券呈折价。' : '收益率接近票息率，价格接近面值。'}</p></div></article>}

      {toolId === 'position' && <article className="tool-calc"><CardTitle id="position" /><div className="tool-fields"><Field id="pp" label="组合总资产" suffix="元" /><Field id="pr" label="单笔可承受损失" suffix="%" /><Field id="ps" label="止损距离" suffix="%" /><Field id="pprice" label="每单位价格" suffix="元" /></div><ToolActions onCalculate={calculate} onReset={reset} /><div className="tool-result coral"><span>最大仓位金额</span><strong>{money(positionValue)}</strong><p>约 {Number.isFinite(positionShares) ? positionShares.toLocaleString('zh-CN') : '—'} 单位 · 若跳空或流动性不足，实际损失可能超过设定值。</p></div></article>}

      {toolId === 'fee-impact' && <article className="tool-calc"><CardTitle id="fee-impact" /><div className="tool-fields"><Field id="fi" label="初始本金" suffix="元" /><Field id="fa" label="每年新增" suffix="元" /><Field id="fr" label="费用前收益率" suffix="%" /><Field id="fy" label="投资年数" suffix="年" /><Field id="f1" label="低费用率" suffix="%" /><Field id="f2" label="高费用率" suffix="%" /></div><ToolActions onCalculate={calculate} onReset={reset} /><div className="tool-result"><span>长期费用差异</span><strong>{money(lowFee - highFee)}</strong><p>低费用终值 {money(lowFee)}；高费用终值 {money(highFee)}。未计税费与跟踪误差。</p></div></article>}
