...
const categoryDepth: Record<string, CategoryDepth> = {
  ...
  '投资基础': {
    ...
    sources: [
      { title: 'What Is Risk?', publisher: 'Investor.gov · U.S. SEC', url: 'https://www.investor.gov/introduction-investing/investing-basics/what-risk', note: '投资风险、收益与不同资产风险来源的基础资料。' },
  ...
  '财务报表': {
    ...
    sources: [
      ...
      { title: 'How to Read a 10-K', publisher: 'Investor.gov · U.S. SEC', url: 'https://www.investor.gov/introduction-investing/getting-started/researching-investments/how-read-10-k', note: '阅读年报、主表与风险披露的实用说明。' },
    ],
  },
  '公司金融与估值': {
  ...
  '组合与风险': {
    ...
    sources: [
      { title: 'Asset Allocation', publisher: 'Investor.gov · U.S. SEC', url: 'https://www.investor.gov/introduction-investing/getting-started/asset-allocation', note: '资产配置、分散和再平衡的基础资料。' },
...
export const toolGuides: Record<string, ToolGuide> = {
  ...
  'early-repay': { question: '有一笔闲钱时，提前还贷还是保留资金投资？', explanation: '工具按当前剩余本金、利率和期数重算等额本息现金流。保留资金方案把可还金额与还款费用投入到期末；提前还款方案把每月降低的月供逐月投入，再比较贷款到期时的资产。', inputs: [{ name: '剩余本金与期数', meaning: '从今天开始尚未偿还的本金和月数，而不是最初贷款金额与期限。' }, { name: '提前还款与费用', meaning: '本次减少的本金，以及合同规定的提前还款费用比例。' }, { name: '贷款利率', meaning: '剩余期间用于计算月供和利息的年利率假设。' }, { name: '替代投资收益', meaning: '扣除预计税费后的情景收益率；它不是确定回报。' }], reading: '同时看新旧月供、扣除提前还款费用后的名义融资成本变化和期末资产差额。投资情景高于贷款成本才可能让保留资金占优，但还要补偿波动、税收与流动性风险。', limits: ['假设贷款利率和剩余期限不变。', '提前还款后按降低月供处理，没有计算保持月供并缩短期限的方案。', '投资收益按固定月收益情景演示，现实回报与顺序并不确定。'], example: '先填入合同中的剩余本金和月份，把替代收益设为较保守的税后值；再加入实际提前还款费用，并确认还款后仍保留足够应急金。' },
  ...
  emergency: { question: '家庭应急金需要覆盖多少个月、准备多少金额？', explanation: '目标等于每月必要支出乘以自己选择的覆盖月数，再加医疗免赔额、紧急维修等已知风险金额。工具只做透明加总，不用收入“等级”或家庭人数生成伪精确建议。', inputs: [{ name: '每月必要支出', meaning: '收入中断后仍必须支付的住房、食品、医疗、保费和最低还款。' }, { name: '自定覆盖月数', meaning: '根据收入来源、重新就业时间、家庭责任和保险自行选择的保护期。' }, { name: '额外应急支出', meaning: '医疗免赔额、必要维修或紧急照护等可能一次发生的资金需求。' }, { name: '现有安全储备', meaning: '可以迅速、低成本并且本金较稳定地动用的资金。' }], reading: '重点看目标金额与现有储备之间的缺口。覆盖月数不是越高越好：安全垫过低会增加借债和被迫卖出风险，过高则会占用长期资金。', limits: ['没有替你估计失业持续时间或事故概率。', '保险赔付时点、双收入相关性和资产到账时间需另行判断。', '度假、学费等日期已知的支出应单独储蓄，不属于应急风险。'], example: '先用真实流水计算必要支出，再选择收入中断保护期；把保险免赔额等额外风险加入，并只将真正可及时动用的安全资产算作现有储备。' },
