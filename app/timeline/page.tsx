    ...
    <main>
      ...
      <section className="timeline-layout">
        <aside>
          ...
          <a href="/atlas/">对照知识地图 →</a>
        </aside>
        <div className="timeline-events">
          {timelineEvents.map((event, index) => (
            <article id={`event-${event.year}`} data-kind={event.kind} key={`${event.year}-${event.title}`}>
              <div><span>{event.year}</span><i /><small>{String(index + 1).padStart(2, '0')}</small></div>
              <section><header><b>{event.kind}</b><small>{index === 0 ? '起点' : `${Number(event.year) - Number(timelineEvents[index - 1].year)} 年后`}</small></header><h2>{event.title}</h2><p>{event.description}</p><blockquote><span>它改变了什么</span>{event.impact}</blockquote></section>
            </article>
          ))}
        </div>
      </section>

      <section className="timeline-end"><span>HISTORY CONTINUES</span><h2>历史不会重复，<br />但机制经常押韵。</h2><p>下一次读到“前所未有”的财经新闻时，试着寻找它对应的旧机制：杠杆、流动性、预期、激励或制度约束。</p><a href="/search/">带着一个问题去检索 →</a></section>
    </main>
  );
}
