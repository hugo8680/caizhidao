            ...
            <div className="video-grid">
              {videos.filter((video) => video.language === language).map((video, index) => (
                <a href={`/videos/${video.id}/`} className="video-card" key={video.id}>
                  <div className="video-screen"><span>{video.platform}</span><HeaderIcon name="play" /><b>{String(index + 1).padStart(2, '0')}</b></div>
                  <div className="video-info"><span>{video.topic} · {video.level}</span><h3>{video.title}</h3>{video.titleEn && <h4>{video.titleEn}</h4>}<p>{video.description}</p><dl><div><dt>平台</dt><dd>{video.platform}</dd></div><div><dt>时长</dt><dd>{video.duration}</dd></div><div><dt>费用</dt><dd>{video.price}</dd></div></dl><b>查看课程详情 <ActionArrow /></b></div>
                </a>
              ))}
            </div>
          </section>
