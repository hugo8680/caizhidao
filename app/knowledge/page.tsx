  ...
  return (
    <main>
      ...
      <section className="knowledge-layout">
        <aside><span>按主题浏览</span>{knowledgeCategories.map((category) => <a href={`#${category}`} key={category}>{category}</a>)}</aside>
        <div className="knowledge-groups">
          {knowledgeCategories.map((category, categoryIndex) => (
            <section className="knowledge-group" id={category} key={category}>
              ...
              <div className="knowledge-list">
                {knowledgeTerms.filter((term) => term.category === category).map((term) => <a href={`/knowledge/${term.slug}/`} id={term.slug} key={term.slug} className="knowledge-card">
